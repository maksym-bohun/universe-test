...
let NatsSubscriberService = NatsSubscriberService_1 = class NatsSubscriberService {
    ...
    async onModuleInit() {
        ...
        await this.subscriber.subscribe('fb.events.top', this.handleFacebookEvent('top'));
        await this.subscriber.subscribe('fb.events.bottom', this.handleFacebookEvent('bottom'));
        this.logger.log('Subscribed to NATS topics');
    }
    handleFacebookEvent(stage) {
        return async (rawEvent) => {
            ...
            this.logger.verbose(`[${correlationId}] Received Facebook ${stage} event`);
            try {
                ...
                this.logger.log(`[${correlationId}] Facebook ${stage} event persisted: ${event.eventId}`);
            }
            catch (error) {
                this.logger.error(`[${correlationId}] Failed to process Facebook ${stage} event`, error);
            }
        };
    }
