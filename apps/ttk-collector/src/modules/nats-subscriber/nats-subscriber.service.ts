...
export class NatsSubscriberService implements OnModuleInit, OnModuleDestroy {
  ...
  async onModuleInit() {
    ...
    await this.subscriber.subscribe(
      'ttk.events.top',
      this.handleTiktokEvent('top'),
    );
    await this.subscriber.subscribe(
      'ttk.events.bottom',
      this.handleTiktokEvent('bottom'),
    );

    this.logger.log('Subscribed to NATS topics');
  }

  private handleTiktokEvent(stage: 'top' | 'bottom') {
    return async (rawEvent: unknown) => {
      ...
      try {
        ...
        this.logger.log(
          `[${correlationId}] Tiktok ${stage} event persisted: ${event.eventId}`,
        );
      } catch (error) {
        this.logger.error(
          `[${correlationId}] Failed to process Tiktok ${stage} event`,
          error,
        );
      }
    };
